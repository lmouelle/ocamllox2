...
let rec eval (env : (string * value) list) (expr : expr) = 
  ...
  let comparison_check lhs rhs f =
    let rec comparison_check' lhs_value rhs_value f =
      ...
      | Variable v, Number n | Number n, Variable v -> comparison_check' (List.assoc v env) (Number n) f
      | Variable v1, Variable v2 -> comparison_check' (List.assoc v1 env) (List.assoc v2 env) f
      | _ -> raise @@ EvalError "Invalid operands for comparison check, only numeric values are valid"
    in
    let lhs_result = eval env lhs in
    let rhs_result = eval env rhs in
  ...
  let equality_check lhs rhs =
    let rec equality_check' lhs_value rhs_value =
      ...
      | Nil, Nil -> true
      | Variable v1, Variable v2 -> equality_check' (List.assoc v1 env) (List.assoc v2 env)
      ...
      | Nil, Number _ | Nil, Boolean _ | Nil , String _ 
      | Number _ , Nil | Boolean _, Nil | String _, Nil -> raise @@ EvalError "Cannot compare value to nil"
      | Number _, Boolean _ | Boolean _, Number _ -> raise @@ EvalError "Cannot compare number to bool"
      | String _, Number _ | Number _, String _ -> raise @@ EvalError "Cannot compare number to string"
      | Boolean _, String _ | String _, Boolean _ -> raise @@ EvalError "Cannot compare string to bool"
    in
    let lhs_result = eval env lhs in
    let rhs_result = eval env rhs in
  ...
  | Equals (lhs, rhs) -> { res = Boolean (equality_check lhs rhs); new_env = env }
  | NotEquals (lhs, rhs) -> { res = Boolean (not @@ equality_check lhs rhs); new_env = env }
  | Less (lhs, rhs) -> {res = Boolean (comparison_check lhs rhs (<)); new_env = env}
  | LessEqual (lhs, rhs) -> {res = Boolean (comparison_check lhs rhs (<=)); new_env = env}
  | Greater (lhs, rhs) -> {res = Boolean (comparison_check lhs rhs (>)); new_env = env}
  | GreaterEqual (lhs, rhs) -> {res = Boolean (comparison_check lhs rhs (>=)); new_env = env}